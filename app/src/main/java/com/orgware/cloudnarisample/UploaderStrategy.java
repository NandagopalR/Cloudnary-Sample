  ...
  public Map callApi(String action, Map<String, Object> params, Map options, Object file)
    ...
    if (Boolean.TRUE.equals(options.get("unsigned"))) {
      ...
      if (options.containsKey("signature") && options.containsKey("timestamp")) {
        ...
        String apiSecret =
            ObjectUtils.asString(options.get("api_secret"), this.cloudinary().config.apiSecret);
    ...
    MultipartUtility multipart =
        new MultipartUtility(apiUrl, "UTF-8", this.cloudinary().randomPublicId(),
            (Map<String, String>) options.get("extra_headers"));
    ...
    if (file instanceof String && !((String) file).matches(
        "(?s)ftp:.*|https?:.*|s3:.*|data:[^;]*;base64,([a-zA-Z0-9/+\n=]+)")) {
    ...
    if (code != 200 && code != 400 && code != 404 && code != 500) {
      throw new RuntimeException(
          "Server returned unexpected status code - " + code + " - " + responseData);
    }
